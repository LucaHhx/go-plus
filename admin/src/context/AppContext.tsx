...
type AppAction =
  | { type: 'NAVIGATE'; page: PageId; params?: Record<string, string> }
...
function loadAuth(): AuthState {
  try {
    ...
    if (saved) {
      ...
      if (parsed.isAuthenticated && parsed.username && parsed.token) {
        return { isAuthenticated: true, username: parsed.username, token: parsed.token };
      }
    }
  } catch { /* ignore */ }
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'LOGIN': {
      const auth = { isAuthenticated: true, username: action.username, token: action.token };
      localStorage.setItem('admin_auth', JSON.stringify(auth));
      return { ...state, auth };
    }
    case 'LOGOUT': {
      localStorage.removeItem('admin_auth');
      return { ...state, auth: { isAuthenticated: false, username: '', token: '' } };
    }
    default:
      return state;
  }
...
export function useAuth() {
  ...
  const login = useCallback((username: string, token: string) => dispatch({ type: 'LOGIN', username, token }), [dispatch]);
...
export function useNavigate() {
  ...
  return useCallback((page: PageId, params?: Record<string, string>) => dispatch({ type: 'NAVIGATE', page, params }), [dispatch]);
}
